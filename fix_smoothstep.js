...
const newPart = `if (shapeType < 0.5) {
        ...
        // IMPORTANT: smoothstep requires edge0 < edge1, so we do 1.0 - smoothstep(small, large)
        float shape = 1.0 - smoothstep(width * 0.1, width, distX);
        ...
        gradient = shape * vFade * mix(0.1, 1.0, bPinch);
...
console.log('Fixed smoothstep inversion bug!');
