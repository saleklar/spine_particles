...
if (!code.includes('const handleSaveCustomPreset')) {
    code = code.replace("const handleCreateFirePreset = useCallback(async (presetType: 'campfire' | 'torch') => {", stateInjection + "  const handleCreateFirePreset = useCallback(async (presetType: 'campfire' | 'torch') => {");
}

const menuCodeOld = \                  <span>Torch</span>
                </button>
              </div>\;
const menuCodeNew = \                  <span>Torch</span>
                </button>
...
if (code.includes(menuCodeOld) && !code.includes('handleSaveCustomPreset()')) {
    code = code.replace(menuCodeOld, menuCodeNew);
} else {
    console.log("Could not find menuCodeOld precisely, or it's already updated.");
}
