...
// Remove old handleExportToParticleSystem declaration block
content = content.replace(/  const handleExportToParticleSystem = useCallback\(\(dataUrls: string\[\]\) => \{[\s\S]*?setSelectedObjectId\(newObject\.id\);\n    \}\n  \}, \[sceneObjects, selectedObjectId\]\);\n/, handleExportCode);
...
"              type=\"button\"\n" +
"            >\n" +
"              {appMode === 'split' ? 'Full Screen 3D' : 'Split View with Particles'}\n" +
"            </button>\n" +
"          </div>\n" +
...
content = content.replace(/  if \(appMode === '3d-animator'\) \{[\s\S]*?<Animator3D onExportToParticleSystem=\{handleExportToParticleSystem\} \/>\n        <\/div>\n      \);\n    \}/, newRenderBlock);

// 4. Add Split View Button to particle system menu
content = content.replace(/        <div className="menu-item">\n          <button\n            className="menu-button"\n            onClick=\{\(\) => setAppMode\('3d-animator'\)\}\n            type="button"\n            style=\{\{ backgroundColor: '#eeb868', color: '#1a1a1a', fontWeight: 'bold' \}\}\n          >\n            3D Asset Creator\n          <\/button>\n        <\/div>/, 
"        <div className=\"menu-item\">\n" +
"          <button\n" +
"            className=\"menu-button\"\n" +
"            onClick={() => setAppMode('3d-animator')}\n" +
