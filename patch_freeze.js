...
const oldFuncBlockStart = 'const generateSequenceDataUrls = async (currentParams: GeneratorParams): Promise<string[]> => {';
const oldFuncBlockEndSearch = '      resolve(dataUrls);\n    });\n  };';

const sIdx = code.indexOf(oldFuncBlockStart);
const eIdx = code.indexOf(oldFuncBlockEndSearch);

if (sIdx !== -1 && eIdx !== -1) {
  let innerCode = code.substring(sIdx, eIdx + oldFuncBlockEndSearch.length);
  ...
  code = code.substring(0, sIdx) + newFuncCode + code.substring(eIdx + oldFuncBlockEndSearch.length);
