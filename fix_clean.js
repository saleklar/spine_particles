...
const m = code.indexOf('return 105.0 * dot( m*m, vec4( dot(p0,x0)');
const s2List = [...code.matchAll(/float fbm\(vec3 x\) \{/g)];

if (m !== -1 && s2List.length > 0) {
   let s2 = s2List[s2List.length - 1].index; // find the last fbm just to be safe it wipes out duplicates
   // Actually let's just find "    return 105.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ) );\n  }"
...
float getNoiseVal(vec3 x) {
   ...
   const s1Match = code.indexOf('}\n\n\nfloat hash(vec3 p) {') !== -1 ? code.indexOf('}\n\n\nfloat hash(vec3 p) {') : code.indexOf('}\n\nfloat hash(vec3 p) {');
   const startIdx = s1Match !== -1 ? s1Match + 1 : code.indexOf('float hash(vec3 p) {');
   
   const fbmIdx = code.indexOf('float fbm(vec3 x) {');

   if (startIdx !== -1 && fbmIdx !== -1) {
       const newCode = code.substring(0, startIdx) + fixedBlock + code.substring(fbmIdx);
       fs.writeFileSync('src/FireGenerator.tsx', newCode);
       console.log('Fixed block successfully');
   } else {
       console.log('Could not find start/end bounds', startIdx, fbmIdx);
   }
