...
// Find start of vertexShader
const vsStart = code.lastIndexOf('export const vertexShader = \');
// Find end of the string literal that has the corrupted shader
const endOfCorruptedShader = code.indexOf('\;', vsStart) + 2;

const correctShaders = \
  ...
  void main() {
      ...
      if (evolveOverLife > 0.5 && distortion < 0.1) {
          a *= (1.0 - pow(loopProgress, 4.0)); // Fade out completely at the end instead of scattering
      }
...
code = code.substring(0, vsStart) + correctShaders + code.substring(endOfCorruptedShader);
