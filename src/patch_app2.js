...
`;
  ...
  const psfps = "particleSpriteSequenceFps: dataUrls.length > 0 ? (presetType === 'campfire' ? defaultCampfireParams.fps : defaultTorchParams.fps) : 30,";
  appCode = appCode.replace(
    "particleSpriteSequenceDataUrls: dataUrls,",
    "particleSpriteSequenceDataUrls: dataUrls,\n        " + psfps
  );
  ...
  const propertiesReplace = "emissionRate: dataUrls.length > 0 ? (presetType === 'campfire' ? 5 : 10) : (presetType === 'campfire' ? 150 : 250),"; 
