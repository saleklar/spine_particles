...
function patch() {
  ...
  const pStart = appCode.indexOf("const handleCreateFirePreset = useCallback((presetType: 'campfire' | 'torch') => {");
  if (pStart === -1) {
  ...
  // Make particles 10 times larger
  customCode = customCode.replace(
    "particleSize: presetType === 'campfire' ? 1.5 : 1.0,",
    "particleSize: dataUrls.length > 0 ? (presetType === 'campfire' ? 30.0 : 20.0) : (presetType === 'campfire' ? 1.5 : 1.0),"
  );

  customCode = customCode.replace(
    "particleType: 'dots',",
    "particleType: dataUrls.length > 0 ? 'sequence' : 'dots',"
  );
  
  // Actually sequence type may not work if logic expects 'sprites' inside particle update, let me check. Actually it is 'sequence'. Oh wait, it is 'sprites'. No wait, looking at `particleType: dataUrls.length > 0 ? 'sequence' : 'dots',`, 
  // Let me replace with 'sprites' because earlier I saw 'sprites' in the enum
