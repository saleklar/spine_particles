...
const s1 = '// For determining gaps where particle is dead/reborn';
const s2 = 'spineData.animations.animation.bones[boneName] = boneAnim;';

const startIndex = c.indexOf(s1);
const endIndex = c.indexOf(s2);
    ...
    const newLogic = `// Chunk history into contiguous life segments
          ...
          for (let i = 0; i < lifespans.length; i++) {
            ...
            if (i > 0 || life[0].frame > 0) {
              slotAnim.rgba.push({ time: (Math.max(0, life[0].frame - 0.01)) / 24, color: "ffffff00" });
            }
