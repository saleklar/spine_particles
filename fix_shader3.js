...
const anchor1 = '}\\n\\n\\nfloat hash(vec3 p) {';
const s1 = code.indexOf(anchor1);
const anchor2 = '\\n\\nfloat fbm(vec3 x) {';
const s2 = code.indexOf(anchor2);
...
float getNoiseVal(vec3 x) {
   ...
   const newCode = code.substring(0, start) + fixedBlock + code.substring(end);
   fs.writeFileSync('src/FireGenerator.tsx', newCode);
   console.log('Fixed block successfully');
} else {
   console.log('Anchors not found. anchor1:', s1, 'anchor2:', s2);
}
