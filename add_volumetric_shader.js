...
const fragStart = code.indexOf('export const fragmentShader = `');
const fragEnd = code.indexOf('\n`;', fragStart) + 3;

const newFragmentShader = `export const fragmentShader = \`
...
void main() {
    ...
    float a = pow(outColor.a, mix(1.5, 0.8, outColor.a));
    
    gl_FragColor = vec4(clamp(c, 0.0, 1.0), clamp(a, 0.0, 1.0));
}
\`;`;

const newCode = code.substring(0, fragStart) + newFragmentShader + code.substring(fragEnd);

fs.writeFileSync('src/FireGenerator.tsx', newCode);
console.log('Volumetric shader installed in FireGenerator.tsx');
