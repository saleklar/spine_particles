...
const targetFunctionStart = '    exportSpineData: () => {';
const targetFunctionEnd = '      return spineData;';

const p1 = code.indexOf(targetFunctionStart);
const p2 = code.indexOf(targetFunctionEnd, p1);
...
let after = code.substring(p2);

// Reconstruct the perfect function body
const replacement = `    exportSpineData: () => {
      if (!sceneSettings.particleBudget) return null;

      const trackData = new Map<string, { frame: number, state: CachedParticleState }[]>();

      // Group states by trackId and emitterId
      for (const [frameObj, states] of cachedParticleHistory.current.entries()) {
      ...
      // Base spine template
      const spineData: any = {
        skeleton: { hash: "0", spine: "4.2.0", x: -500, y: -500, width: 1000, height: 1000 },
        bones: [
          { name: "root" }
        ],
        slots: [],
      ...
      trackData.forEach((history, trackIdStr) => {
        ...
        spineData.slots.push({ name: slotName, bone: boneName, attachment: null }); // explicitly null by default so no lingering images
        
        let skinAttachments = spineData.skins[0].attachments;
        
        // Find sequence count for this particle's emitter using the unique map
        let seqInfo = null;
        if (history.length > 0) {
            seqInfo = emitterSequences.get(history[0].state.emitterId);
        }
...
console.log('Replaced whole function beautifully!');
