              ...
              {project.object.geometry === 'coin' && (
                <>
                  ...
                  <select
                    ...
                    onChange={(e) => setProject(prev => ({ ...prev, object: { ...prev.object, geometryParams: { ...prev.object.geometryParams, coinInnerShapePattern: e.target.value } } }))}
                    style={{ width: '100%', padding: '6px', marginBottom: '8px' }}
                  >
                  ...
                  {project.object.geometryParams.coinInnerShapePattern && project.object.geometryParams.coinInnerShapePattern !== 'none' && (
                    <>
                      ...
                      {(project.object.geometryParams.coinInnerShapePattern === 'polygon') && (
                        <>
                          <label style={{ display: 'block', marginBottom: '4px', fontSize: '12px' }}>Roundness</label>
                          <input
                            type="number" value={project.object.geometryParams.coinInnerShapeRoundness ?? 0}
                            onChange={(e) => setProject(prev => ({ ...prev, object: { ...prev.object, geometryParams: { ...prev.object.geometryParams, coinInnerShapeRoundness: Number(e.target.value) } } }))}
                            style={{ width: '100%', padding: '6px', marginBottom: '8px' }}
                            step="0.1"
                          />
                        </>
...
code = code.replace(
  /\{\s*project\.object\.geometry === 'cylinder' && \(/,
